...
process_rpc_request(Socket, {call, Mod, Fun, Args}) ->
    try
        ...
        Type:Reason ->
            ...
            gen_tcp:send(Socket, term_to_binary(format_error(Mod, Fun, Args)))
    after
...
format_error(Mod, Fun, Args) ->
    ...
    {Code, Reason} = case code:ensure_loaded(Mod) of
        {module, Mod} ->
            {2, list_to_binary(io_lib:format("function ~p/~p not found on module ~p", [Fun, length(Args), Mod]))};
        {error, nofile} ->
            {1, list_to_binary(io_lib:format("module ~p does not exists", [Mod]))}
    end,
